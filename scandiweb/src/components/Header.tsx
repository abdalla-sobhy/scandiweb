import { useEffect, useState } from "react"; import IndexCSS from "../../public/styles/index.module.css"; import { useMutation } from '@apollo/client'; import { PLACE_ORDER } from '../queries';

interface HeaderProps { category: string; setCategory: (category: "all" | "clothes" | "tech") => void; toggleWheneAddToCart: boolean; itemAdded: boolean; }

interface productsInfo { product_id: string; name: string; price: number; image: string; category: "all" | "clothes" | "tech"; size: "XS" | "S" | "M" | "L"; color: "gray" | "black" | "green"; quantity: number; }
                    ...
                    <div className={`${cartProducts.length !== 0 ? IndexCSS.showCart : IndexCSS.showCartEmpty}`}>
                        <div className={`${cartProducts.length !== 0 ? IndexCSS.showCartContents : IndexCSS.showCartContentsEmpty}`}>
                            ...
                            <div className={`${IndexCSS.cartCardContainer}`} key={`${product.product_id}-${product.size}-${product.color}`}>
                                <div className={`${IndexCSS.cartCardContents}`}>
                                                ...
                                                {[
                                                    ...
                                                    <div key={color} className={`${IndexCSS.colorChoiceFirstLayer}`}>
                                                        <div 
                                                            ...
                                                            className={`${IndexCSS.colorBox} ${product.color === color ? 'outline-2 outline-[#5ECE7B] outline-offset-1' : ''}`} 
                                    ...
                                    <div className={`${IndexCSS.cartCardMiddlePart} flex flex-col justify-between items-center`}>
                                        ...
                                        >
                                            <img src="../../src/assets/plus-square.svg" alt="" />
                                        </div>
                                    ...
                                    <div className={`${IndexCSS.cartCardImage}`}>
                                        <img className="h-full w-full" src={product.image.replace(/[[\]"]/g, '')} alt="" />
                                    </div>
                                </div>
                            </div>
                            ))}
                            <div className="w-full flex flex-row justify-between">
                                ...
                                <div data-testid="cart-total" className={`${IndexCSS.cartTotalPrice}`}>
                            ...
                            <div className={`${IndexCSS.placeOrderButtonDiv}`}>
                                <button 
                                    onClick={cartProducts.length !== 0 ? handlePlaceOrder : undefined} 
                                    className={`${cartProducts.length !== 0 ? IndexCSS.placeOrderButtonDiv : IndexCSS.placeOrderButtonDivEmpty} w-full h-full cursor-pointer`}
                                >
                                    PLACE ORDER
                                </button>
                            </div>
