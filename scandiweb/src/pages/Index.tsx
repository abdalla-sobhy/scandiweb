import { useState } from 'react'; import { useQuery } from '@apollo/client'; import { GET_PRODUCTS } from '../queries'; import IndexCSS from '../../public/styles/index.module.css'; import { useNavigate } from 'react-router-dom'; import { useProduct } from "../components/ProductContext"; import Header from "../components/Header";

type Category = "all" | "clothes" | "tech";

interface productsInfo { id: string; name: string; price: number; gallery: string[]; category: Category; inStock: boolean; }

interface OrderItem { product_id: string; name: string; price: number; image: string; size: 'XS' | 'S' | 'M' | 'L'; color: 'gray' | 'black' | 'green'; category: Category; quantity: number; }

export default function Index() { const { setSelectedProductId } = useProduct(); const [toggleWheneAddToCart, setToggleWheneAddToCart] = useState<boolean>(false); const [category, setCategory] = useState<Category>("all"); const [itemAdded, setItemAdded] = useState<boolean>(false); const navigateTo = useNavigate();
                                ...
                                >
                                    {!product.inStock && (
                                        <div className={IndexCSS.outOfStockImage}>
                                            <img src='../../src/assets/out of stock.svg' alt='out of stock' />
                                        </div>
                                    )}
