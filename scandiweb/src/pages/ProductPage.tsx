import { useState } from "react"; import { useQuery } from "@apollo/client"; import { GET_PRODUCT_BY_ID } from "../queries"; import Header from "../components/Header"; import productPageCSS from "../../public/styles/ProductPage.module.css"; import { useProduct } from "../components/ProductContext"; import { useNavigate } from 'react-router-dom'; import { useSearchParams } from 'react-router-dom'; import parse from 'html-react-parser';

type Category = "all" | "clothes" | "tech";

type productColor = 'gray' | 'black' | 'green'; type productSize = 'XS' | 'S' | 'M' | 'L';

interface Product { id: string; name: string; price: number; gallery: string[]; category: Category; description: string; inStock: boolean; attributes: { id: string; name: string; type: string; items: { id: string; displayValue: string; value: string }[]; }[]; }

interface OrderItem { product_id: string; name: string; price: number; image: string; size: productSize; color: productColor; category: Category; quantity: number; }

export default function ProductPage() { const { selectedProductId } = useProduct(); const [currentImageIndex, setCurrentImageIndex] = useState(0); const [toggleWheneAddToCart, setToggleWheneAddToCart] = useState<boolean>(false); const [size, setSize] = useState<productSize>('XS'); const [color, setColor] = useState<productColor>('gray'); const [itemAdded, setItemAdded] = useState<boolean>(false); const [searchParams] = useSearchParams(); const categoryFromQuery = (searchParams.get("category") as "all" | "clothes" | "tech"); const navigateTo = useNavigate(); const { data, loading, error } = useQuery(GET_PRODUCT_BY_ID, { variables: { id: selectedProductId }, });
                ...
                <div className={`${productPageCSS.rightPart}`}>
                    <div className={`${productPageCSS.rightPartContents} flex flex-col gap-8 ml-12`}>
                        ...
                        <div className="flex flex-col items-start gap-2">
                            ...
                            <div data-testid="product-attribute-color" className="flex flex-row gap-2">
                                <div className={`${productPageCSS.colorChoiceFirstLayer}`}>
                                    <div 
                                        className={`${productPageCSS.colorBox} bg-[#D3D2D5] ${color === 'gray' && ' outline-2 outline-[#5ECE7B] outline-offset-1'}`} 
                                        onClick={()=>setColor('gray')}
                                    ></div>
                                </div>
                                <div 
                                    className={`${productPageCSS.colorBox} bg-[#2B2B2B] ${color === 'black' && ' outline-2 outline-[#5ECE7B] outline-offset-1'}`} 
                                    onClick={()=>setColor('black')}
                                ></div>
                                <div 
                                    className={`${productPageCSS.colorBox} bg-[#0F6450] ${color === 'green' && ' outline-2 outline-[#5ECE7B] outline-offset-1'}`} 
                        ...
                        <div className="flex flex-col items-start gap-4">
                            ...
                            <div className={`${productPageCSS.sizeAndColorAndPriceWordCSS}`}>
                                ${product?.price.toFixed(2)}
                            </div>
                        </div>
                        <div className={`${productPageCSS.addToCartButtonDiv} ${!product.inStock ? 'disabled' : ''}`}>
                            <button 
                        ...
                        <div data-testid="product-description" className={`${productPageCSS.underButtonText}`}>
